-- MAYHEM local database v5: additive Feed vNext foundation.
-- Keep statements separated by the marker consumed by the deterministic export.
...
-- statement
CREATE TABLE IF NOT EXISTS feed_assignments (
...
-- statement
CREATE UNIQUE INDEX IF NOT EXISTS feed_assignments_batch_position_idx
...
-- statement
CREATE UNIQUE INDEX IF NOT EXISTS challenge_attempts_one_open_idx
ON challenge_attempts ((1))
WHERE status = 'active'

-- statement
CREATE INDEX IF NOT EXISTS challenge_attempts_history_idx
...
-- statement
CREATE INDEX IF NOT EXISTS event_log_v2_sync_idx
ON event_log_v2 (sync_status, next_retry_at)

-- statement
CREATE INDEX IF NOT EXISTS event_log_v2_attempt_idx
ON event_log_v2 (attempt_id, event_type)

-- statement
CREATE INDEX IF NOT EXISTS event_log_v2_occurred_idx
...
-- statement
CREATE TABLE IF NOT EXISTS feature_flags_cache (
